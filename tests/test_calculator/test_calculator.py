...
def test_show_history(setup_history):
    '''Tests that show history function works'''
    assert Calculator.show_history() == [(2, 2, 'add'), (2, 2, 'subtract')], 'Failed to retrieve history!'

def test_show_previous(setup_history):
    '''Tests that show previous function works'''
    assert Calculator.show_previous() == (2, 2, 'subtract'), 'Failed to retrieve previous log!'

def test_clear_history(setup_history):
    '''Tests that clear history function, and show history and show previous functions works for no history'''
    Calculator.clear_history()
    assert len(History.history) == 0, 'Failed to clear history!'
    assert Calculator.show_history() == 'No History!', 'Failed to show no history!'
    assert Calculator.show_previous() == 'No History!', 'Failed to show no previous history!'
